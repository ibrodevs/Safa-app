      ...
      child: RefreshIndicator(
        ...
        child: Consumer<ShipmentsHistoryProvider>(
          builder: (context, state, _) {
            ...
            // Пустой список — только красивая надпись
            if (!state.loading && state.items.isEmpty) {
              return ListView(
                ...
                return _HistoryCard(
                  ...
                  onDetails: () {
                    final data = HistoryDetailsData(
                      titleNumber: numberLabel,
                      productTitle: item.title,
                      chips:
                      chips.map((c) => ChipData(c.asset, c.text)).toList(),
                      registeredAt: item.createdAt,
                      routeText: item.pickup,
                      compositionText:
                      'Размер: ${item.sizeLabel}\nКоличество: ${item.quantity}',
                      flightNumber: item.publicCode,
