...
void main() {
  test('client and specialist use compact hierarchy and server fare', () {
    final nearby = File(
      'lib/features/carrier_module/home/data/model/nearby_shipment.dart',
    ).readAsStringSync();
    final carrier = File(
      'lib/features/carrier_module/home/carrier_home_screen.dart',
    ).readAsStringSync();
    final point = File(
      'lib/features/main_module/map/data/model/delivery_point_model.dart',
    ).readAsStringSync();
    final summary = File(
  ...
  test('specialist map keeps backend-authored drawing layers disabled', () {
    ...
    expect(map, contains('backendDrawingLayersEnabled = true'));
  });

  test(
    'specialist empty-order refresh does not recursively restart polling',
    () {
      final source = File(
        'lib/features/carrier_module/home/carrier_home_screen.dart',
      ).readAsStringSync();
      ...
      expect(refreshBody, isNot(contains('_startNearbyPolling()')));
      expect(refreshBody, contains('if (_nearby.isEmpty && _showEmptyOrders)'));
    },
  );
